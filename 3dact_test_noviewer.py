...
DEFAULT_MODEL_ID = "../outputs/presentation/train3d4k"
...
def is_valid_start_state():
    return np.linalg.norm(get_box_pose() - get_target_pose()) > 0.12
...
with torch.inference_mode():
    ...
    while not is_valid_start_state():
        observation, info = env.reset(seed=config['seed'])
        randomize_task()
    while state.episode_number <= MAX_EPISODES:
        ...
        if state.success or state.step_count > MAX_STEPS_PER_EPISODE or key_states.get('X', False):
            ...
            if save_point_clouds:
                visualize_pointclouds_video(
                    point_cloud_frames, path=f'{save_dir}/episode_{state.episode_number}_cloud.html', fps=50)
...
print(f"Average FPS: {np.mean(total_fps/len(episode_results)):.2f}")
print(f"Total is_picked_up: {sum(result['is_picked_up'] for result in episode_results)}")
print(f"Total is_placed: {sum(result['is_placed'] for result in episode_results)}")
print(f"Total success: {sum(result['success'] for result in episode_results)}")
print(f"Total episodes: {len(episode_results)}")

with open(os.path.join(save_dir, 'results.json'), 'w') as f:
    json.dump({'average_fps': np.mean(total_fps/len(episode_results))}, f)
