...
class PointCloudGenerator(object):
    ...
    @param max_bound: If not None, list len(3) containing largest x, y, and z
        values that will not be cropped
    """
    def __init__(self, model, cam_ids:List, img_size=84):
        ...
        for cam_id in range(len(self.cam_ids)):
            # get camera id
    ...
    def generateCroppedPointCloud(self, renderer, color_renderer, save_img_dir='../../../evaluation_videos/pc_debug/'):
        o3d_clouds = []
        cam_poses = []
        depths = []
        for cam_id in self.cam_ids:
            ...
            color_img, depth = self.captureImage(renderer, color_renderer, capture_depth=True)
            depths.append(depth)
            # If directory was provided, save color and depth images
            #    (overwriting previous)
            if save_img_dir != None:
                self.saveImg(depth, save_img_dir, "depth_test_" + str(cam_id))
                self.saveImg(color_img, save_img_dir, "color_test_" + str(cam_id))
            ...
            cam_body_id = self.model.cam_bodyid[cam_id]
            cam_pos = self.model.body_pos[cam_body_id]
            c2b_r = rotMatList2NPRotMat(self.model.cam_mat0[cam_id])
