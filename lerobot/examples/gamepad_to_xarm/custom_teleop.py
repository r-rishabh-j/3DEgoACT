...
print("Controls: Arrow keys (x,y), K/L (z), Space (close), Shift (open)")
print("Press S to start recording, R to save episode, X to exit")
...
def randomize_task():
    ...
    # Randomize object
    obj_x = np.random.uniform(1.3, 1.7)
    obj_y = np.random.uniform(-0.2, 0.4)
    data.qpos[object_qpos_adr:object_qpos_adr+3] = [obj_x, obj_y, 0.68]
    
    # Randomize target
    tgt_x = np.random.uniform(1.3, 1.7)
    tgt_y = np.random.uniform(-0.2, 0.4)
...
#     print(f"[DEBUG] Right Key State: {right_key_state}")
