...
class XboxXarmController(InputController):
    """Generate motion deltas from gamepad input."""

    def __init__(self, x_step_size=1.0, y_step_size=1.0, z_step_size=1.0, deadzone=0.15):
        super().__init__(x_step_size, y_step_size, z_step_size)
        self.deadzone = deadzone
        self.intervention_flag = False
    ...
    def apply_deadzone(self, value):
        ...
        return (value - self.deadzone * (1 if value > 0 else -1)) / (1 - self.deadzone)
    ...
    def get_deltas(self):
        ...
        for i in range(num_axes):
            state[f'axis_{i}'] = self.apply_deadzone(self.joystick.get_axis(i))
        ...
        for i in range(num_buttons):
            state[f'button_{i}'] = self.joystick.get_button(i)
        
        # Optional: Get hats (D-pads) if needed
        num_hats = self.joystick.get_numhats()
        for i in range(num_hats):
            state[f'hat_{i}'] = self.joystick.get_hat(i)

        terminate = state['button_2']
        record = state['button_0']
        
        return state,terminate,record
