...
class MujocoPointcloudXArm:
    """
    fetch point cloud from mujoco and add it to obs
    """
    def __init__(self, model, data, cam_ids, img_size, env_name:str='xarm', use_point_crop=True):
        ...
        self.pc_generator = PointCloudGenerator(model, data, cam_ids, img_size)
    ...
    def get_point_cloud(self, depth, color_img, use_RGB=True):
        save_img_dir = None
        point_cloud, depth = self.pc_generator.generateCroppedPointCloud(depth, color_img) # (N, 6), xyz+rgb
